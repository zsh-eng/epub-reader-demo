...
@main struct AnnotationStoreChecks {
  @MainActor static func main() throws {
    ...
    precondition(restored.annotations(for: url).isEmpty)
    ...
    precondition(partial.records.count == 1)
    precondition(partial.loadError != nil)
    print(
      "Annotation storage: reopen, Unicode, deduplication, note preservation, deletion, stale merge, damaged-record isolation passed"
    )
  }
}
