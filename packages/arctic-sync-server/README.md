...
This backend is mounted in `apps/reader/server/index.ts` under `/api/arctic`. `apps/arctic/Sync` is a
...
`articles` scheme in `apps/arctic/Info.plist` already covers it. The helper
...
Add the local Swift package `apps/arctic/Sync` and its `ArcticSync` product to ArticleReader.
...
```
bun test packages/arctic-sync-server/routes.test.ts
