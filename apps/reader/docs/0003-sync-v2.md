...
Define the new table in [server/db/schema.ts](../server/db/schema.ts) and generate a new Drizzle migration.
...
Add versioned routes in [server/index.ts](../server/index.ts), while the old `/api/sync/:table` routes remain active:
...
The retired package's D1 conformance suite was the original reference. The production [sync-v2.test.ts](../test/server/sync-v2.test.ts) endpoint suite should test:
...
Replace the existing provider and service usage starting at [App.tsx](../src/App.tsx). Keep the current public hook shape where this reduces application changes.
