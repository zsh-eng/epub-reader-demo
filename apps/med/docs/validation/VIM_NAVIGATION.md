...
Supported motions: `h j k l`, counts, `w b e`, `0 ^ $ A`, `f t F T`, `; ,`, `gg G nG`, `Ctrl-D / Ctrl-U`, `zz zt zb`, and paragraph `{ }` with counts. Paragraph boundaries are empty lines (including CRLF), not whitespace-only lines. `Shift+A` moves to the final character and stays in read-only normal mode. `zz`, `zt`, and `zb` keep the cursor column and place its line at the middle, top, or bottom of the viewport, within the file’s scroll limits. `zt` and `zb` leave four lines of space above or below the cursor; very short viewports reduce this margin. A preceding count selects the target line. `:<line>` opens a line-address prompt: Enter moves to the first column and centers the line; Escape leaves the cursor and scroll position unchanged. Only positive whole line numbers are accepted; values beyond the file end select its last line. Insert mode, operators, visual mode, registers, and macros are out of scope.
